...
with col1:
    ...
    if submitted:
        # สร้าง DataFrame จากข้อมูลที่ผู้ใช้กรอก (ต้องเรียงตาม original_features)
        ...
        input_data = input_data.reindex(columns=original_features, fill_value='FALSE')
        ...
        gross_profit_estimate = prediction - cost_amount
        if gross_profit_estimate > 0:
            st.success(f"✅ **กำไรโดยประมาณ:** ฿ {gross_profit_estimate:,.2f} (ยอดขายหักต้นทุน)")
        else:
            st.warning(f"⚠️ **ขาดทุนโดยประมาณ:** ฿ {gross_profit_estimate:,.2f} (ควรปรับลดต้นทุนหรือเพิ่มยอดขาย)")
    ...
    if submitted:
        summary_df = pd.DataFrame({
            'รายการ': [
                ...
                'วันหยุดสุดสัปดาห์', 'มีการคืนสินค้า', 'วิธีการชำระเงิน',
                'วันในสัปดาห์', 'คะแนนความพึงพอใจ'
            ],
            'ค่า': [
                ...
                is_weekend, returned, payment_method,
                day_of_week, satisfaction_score
            ]
        })
